 <!--image gallery section start-->
   
        <section class="image-gallery-section gray-light-bg ptb-20">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-md-8 col-lg-8">
                        <div class="section-heading text-center mb-3">
                            <h2>Elite Vendors, Exceptional Experiences!</h2>
                          
                        </div>
                    </div>
                </div>
                <div class="row image-gallery">
                 
                <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="#" title="Winter Dance">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
					
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="#" title="Winter Dance">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
					
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="#" title="The Uninvited Guest">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
							
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="#" title="Oh no, not again!">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
							
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="assets/img/hero-bg5.jpg" title="Swan Lake">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
							
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="assets/img/hero-bg6.jpg" title="The Shake">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
						
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="assets/img/hero-bg7.jpg" title="Who's that, mommy?">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
							
                        </div>
                    </div>
                    <div class="col-md-6 col-lg-3 gallery-imgborder">
                        <div class="gallery-img">
                            <a href="assets/img/hero-bg8.jpg" title="Who's that, mommy?">
                                <img src="<?php echo e(asset('frontassets/img/vendor.jpg')); ?>" class="img-fluid rounded" alt="gallery image">
                            </a>
						
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!--image gallery section end--><?php /**PATH C:\xampp\htdocs\mangalmall\resources\views/layouts/vendorgallery.blade.php ENDPATH**/ ?>