            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="text-end">
                              <a href = "{{ route('invitationsize.create') }}" class="btn btn-primary waves-effect waves-light mb-4 text-end"><span class="tf-icon mdi mdi-plus me-1"></span> Add </a>
                        </div>
                         ...
                         <div class="table-responsive">
                            ...
                            <table class="table mb-0">
                                ...
                                <tbody>
                                    @if(count($invitationsize) > 0)
                                    @foreach($invitationsize as $typename)
                                    <tr>
                                        <th scope="row">{{  $i++ }}</th>
                                        <td>{{ $typename->sizename }}</td>           
                ...
                @endif
                <a href="{{ url('/admin/invitation/invitationsize/'.$typename->id.'/edit') }}" class="btn-warning btn" title="Edit"><i class="fa fa-pencil action_icon"></i>
                </a>
                 <button type="button" class="btn-danger btn deleteid" data-bs-toggle="modal"  data-bs-target="#delModal" data-id="{{ $typename->id }}" title="Delete"  >
                    <i class="fa fa-trash action_icon"></i>
                                    ...
                                    @endforeach
                                       {{ $invitationsize->links('pagination::bootstrap-4') }}
           @else
                No Records Found
        @endif
                                </tbody>
...
@endsection
<input type="hidden" name="redirecturl" id="redirecturl" value="{{ url('/admin/invitation/invitationsize/') }}">  
