         ...
         <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <form class="form-horizontal" role="form" method = "post" action="{{ route('venue.city_add') }}">
                        @csrf
                        <div class="col-6">
                        <div class="mb-4 row">
                            <label class="col-md-4 col-form-label" for="cityname">City Name</label>
                            <div class="col-md-8">
                                <input type="text" id="cityname" name="cityname" class="form-control" placeholder="Enter the City name" value = "{{ old('cityname') }}" >
                                @error('cityname')
                       ...
                       <div class="mb-4 row">
                           ...
                           <div class="col-md-8">
                               <select name="districtid" class="form-control" id="districtDropdown">
                                   ...
                                   <option value="{{ $dis->id }}" {{ old('districtid') ==  $sta->id ? 'selected' : '' }} > {{$dis->districtname  }}</option>
                                   @endforeach
                               </select>
                               @error('districtid')
                               <div class="text-danger">{{ $message }}</div>
