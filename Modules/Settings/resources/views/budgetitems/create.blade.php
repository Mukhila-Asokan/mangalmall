...
<div class="col-12">
    <div class="card">
        ...
        <form class="form-horizontal" role="form" method = "post" action="{{ route('admin.budgetitems.store') }}">
            @csrf
            <div class="col-6">
            <div class="mb-4 row">
                <label class="col-md-4 col-form-label" for="name">Budget Item Name</label>
                <div class="col-md-8">
                    <input type="text" id="name" name="name" class="form-control" placeholder="Enter the Budget Item name" value="{{ old('name') }}">
                    @error('name')
                ...
                <div class="col-md-8">
                    <select id="budget_category_id" name="budget_category_id" class="form-control">
                        ...
                        @foreach($budgetCategories as $category)
                            <option value="{{ $category->id }}" {{ old('budget_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
