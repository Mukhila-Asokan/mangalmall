...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            <h4 class="header-title mb-4">Edit Event Budget</h4>
        ...
        <form class="form-horizontal" role="form" method = "post" action="{{ route('admin.eventbudget.update', $eventBudget->id) }}">
            @csrf
            @method('PUT')
            <div class="col-6">
                ...
                <div class="col-md-8">
                    <select id="occasion_id" name="occasion_id" class="form-control">
                        ...
                        @foreach($occasions as $occasion)
                            <option value="{{ $occasion->id }}" {{ $eventBudget->occasion_id == $occasion->id ? 'selected' : '' }}>{{ $occasion->eventtypename }}</option>
                        @endforeach
                    </select>
                    @error('occasion_id')
                ...
                <div class="col-md-8">
                    <select id="category_id" name="category_id" class="form-control">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ $eventBudget->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                    <div class="text-danger">{{ $message }}</div>
            ...
            <br><br>
                <div class="justify-content-end row">
                    <div class="col-sm-9">
                        <button type="submit" class="btn btn-primary waves-effect waves-light">Update Event Budget</button>
                    </div>
                </div>
            </div>
        </form>
