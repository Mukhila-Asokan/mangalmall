...
<div class="container py-5">
        ...
        <form id="wizardForm" method = 'post' action = "{{ route('user.webpage.store') }}">
        @csrf
            ...
            <label for="occasion_type"><h6> <span class="mdi mdi-seat"></span> Step 1: Choose Occasion Type</h6></label>
                <select class="form-control mb-3 has-value" name="occasion_type" id="occasion_type" required>
                    ...
                    @foreach($occasiontype as $type)
                        <option value="{{ $type->id }}" {{ old('occasion_type') == $type->id ? 'selected' : '' }}>{{ $type->eventtypename }}</option>
                    @endforeach
                </select>
            </div>
            </div>
