...
<div class = "col-md-6 col-6">
    ...
    <h4 class="mt-5 mb-5 text-center"> {{ $userOccasion->Occasionname->eventtypename  }}</h4>                       
    ...
    <div class="card p-4 mb-4 shadow-sm">
        ...
        <div class="d-flex justify-content-between p-2 bg-light rounded">
            <span class="fw-bold text-primary">
                <input type = "hidden"  id = "totalplanned-amount" value = "{{ $planned_amount }}">  
                <input type = "hidden"  id = "totalcompleted-amount" value = "{{ $completed_amount }}">
                Planned Amount: <span id="total-planned">{{ $planned_amount }}</span>
            </span>
            <span class="fw-bold text-success">
                Actual Amount: <span id="total-actual">{{ $completed_amount }}</span>
            </span>
...
<hr class="my-3">
 ...
 <ul class="sortable-list list-unstyled">
    ...
    <li class="checklist-item card p-3 mb-3 d-flex align-items-center shadow-lg position-relative" data-id="{{ $bud->id }}">
    <div class="d-flex flex-column w-100">
    <div class="d-flex justify-content-between align-items-center">
        <span>
            <h5 class="text-primary">
                <strong>Budget Name:</strong> {{ $bud->name ?? 'N/A' }}
            </h5>
        </span>
    ...
    <div class="d-flex justify-content-between align-items-center">
        <span>
            ...
            <span id="planned-amount-{{ $bud->id }}" class="planned-amount">{{ $bud->planned_amount ?? '0' }}</span>
        </span>

        <span>
            <strong>Actual Amount:</strong>
            <span id="actual-amount-{{ $bud->id }}" class="actual-amount">{{ $bud->completed_amount ?? '0' }}</span>
        </span>
        ...
        <i class="fas fa-edit edit-icon text-primary"
           ...
           data-id="{{ $bud->id }}"
           data-planned="{{ $bud->planned_amount }}"
           data-actual="{{ $bud->completed_amount }}"
        ...
        <div class="homemodal-content">
            ...
            <div class="modal-body">
                <form id="addBudgetForm" method="POST">
                    ...
                    <!-- Hidden Occasion ID -->
                    <input type="hidden" name="useroccasion_id" value="{{ $userOccasion->id ?? '' }}">
...
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="homemodal-content">
            ...
            <div class="modal-body">
                <form id="editAmountForm" method="POST" action="{{ route('userbudget.update') }}">
                    @csrf
        ...
        if (e.target.classList.contains('delete-icon')) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch(`{{ route('userbudget.destroy', '') }}/${budgetId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
                        }
