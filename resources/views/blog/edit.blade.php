                     ...
                     <div class="row">
                          <div class="col-md-8">  
                                <div class="mb-3">
                                     ...
                                     <input type="text" class="form-control" id="blogtitle" name="title" placeholder="Enter the Blog Title" value="{{ old('title', $blog->title) }}">
                                     @error('title')
                                ...
                                <div class="mb-3">
                                     ...
                                     <textarea class="form-control" id="blogcontent" placeholder="Enter the Blog Content" name="content" style="height:600px;">{{ old('content', $blog->content) }}</textarea>
                                     @error('content')
                                          <div class="text-danger">{{ $message }}</div>
                                     @enderror
                                </div>
                          ...
                          <div class="col-md-4">  
                                <div class="mb-3 text-center">
                                     <img id="imagePreview" src="{{ $blog->image ? asset('storage/' . $blog->image) : asset('frontassets/img/preview.jpg') }}" alt="Mangal Mall Image Preview" style="width:350px"/>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                     ...
                                     <select class="form-control" id="blogcategory" name="category">
                                          ...
                                          <option value="{{ $category->id }}" {{ old('category', $blog->category_id) == $category->id ? 'selected' : '' }}>{{ $category->categoryname }}</option>
                                          @endforeach
                                     </select>
                                     @error('category')
                                          <div class="text-danger">{{ $message }}</div>
                                ...
                                <div class="mb-3">
                                     ...
                                     <select class="form-control" id="blogstatus" name="blogstatus">
                                          ...
                                          <option value="rejected" {{ old('blogstatus', $blog->status) == 'rejected' ? 'selected' : '' }}>Trash</option>						
                                     </select>
                                </div>	
...
<script>
    ...
    var defaultTags = @json(explode(',', $blog->tags));

    var tagify = new Tagify(input, {
        whitelist: @json($tags->pluck('tagname')->toArray()), // Available tags
        dropdown: {
            maxItems: 5,
