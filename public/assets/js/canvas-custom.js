(function( $ ) {
	...
	canvas.setBackgroundImage(
		'data:image/gif;base64,',
	...
	$(function() {
		...
		$('.get_canvas_image').on('click', function(e) {
		    ...
		    canvas.loadFromJSON(JSON.stringify(canvas), function() {
		        ...
		        var extension = '';		        
		        if (format == 'image/png') {
		            fileData = canvas.toDataURL({ format: 'png' });
		            extension = 'png';
		        } else if (format == 'image/jpeg') {
		            fileData = canvas.toDataURL({ format: 'jpeg', quality: 0.8 });
		            extension = 'jpg';
		        }
		        var ad = $("<a>").attr("href", fileData).attr("download", "template." + extension).appendTo("body");
		        ad[0].click();
		        ad.remove();
		    });
		    setTimeout(function() {
		        $('.mangalmall-loader').hide(); // Hide loader after file generation
		    }, 1000);
		});
				...
				success: function(data){
					...
					if(result.status){
						jQuery.each(result.data, function(k, v) {
							$('.pg-image-list.pg-recent-uploaded.bg').append('<div class="pg-imglist-item"><div class="ed_image pg_canvas_bg_image" data-url="/storage/'+v.image_url+'"><img src="/storage/'+v.thumb_url+'" alt=""></div><span class="pg-remove-image pg_canvas_bg_remove_image" data-id="'+v.id+'"></span></div>');
							$('.pg-image-list.pg-recent-uploaded.image').append('<div class="pg-imglist-item"><div class="ed_image pg_canvas_add_image" data-url="/storage/'+v.image_url+'"><img src="/storage/'+v.thumb_url+'" alt=""></div><span class="pg-remove-image pg_canvas_add_remove_image" data-id="'+v.id+'"></span></div>');
						});
		...
		console.log('Page '+page);	
			let csrfToken = document.querySelector('meta[name="_token"]') 
				? document.querySelector('meta[name="_token"]').getAttribute('content') 
				: null;
		
			if (!csrfToken) {
				console.error("CSRF token not found. Ensure it's included in your HTML.");
				return;
			}
		
			fetch(ajaxurl + '/cardinvitation/moreImages', {
				method: "POST",
				headers: {
			...
			.then(result => {
				...
				if (result.status) {
						...
						if (bgContainer) {
							bgContainer.insertAdjacentHTML('beforeend', `
								<div class="pg-imglist-item">
									...
									<span class="pg-remove-image pg_canvas_bg_remove_image" data-id="${v.id}"></span>
								</div>
							`);
						}
						...
						if (imgContainer) {
							imgContainer.insertAdjacentHTML('beforeend', `
								<div class="pg-imglist-item">
									...
									<span class="pg-remove-image pg_canvas_add_remove_image" data-id="${v.id}"></span>
								</div>
							`);
						}
					});
					...
					this.style.display = "none"; // Hide load more button if no more images
				}
			})
		...
		*/
		...
		if($('#template_id').val() != ''){
			...
			$.ajax({
				type:'post',
				url: ajaxurl + '/home/invitationcard/getObject',
				headers: {
					'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
						},
				data:{'template_id':template_id, 'sub_user_id':sub_user_id},
				success:function(data){
					console.log(typeof data); // Check if it's already an object
        			console.log(data);
					 var result = data;
	...
	/*function upload_image( data, type ){
		...
		var xhr = $.ajax({
			url: ajaxurl + '/cardinvitation/uploadMedia', 
			type: "POST",         
			data: form_data, 
			contentType: false,       
			cache: false,            
		...
		function upload_image(data, type) {
			...
			let csrfToken = document.querySelector('meta[name="_token"]') 
				? document.querySelector('meta[name="_token"]').getAttribute('content') 
				: null; // Check if CSRF token exists
			...
			fetch(ajaxurl + '/cardinvitation/uploadMedia', {
				method: "POST",
				body: form_data,
				headers: {
					"X-CSRF-TOKEN": csrfToken
			...
			.then(result => {
				if (result.success) {
					...
					if (type === 'bg' && bgContainer) {
						...
						`);
					} else if (type === 'image' && imgContainer) {
					...
					let errorContainer = document.querySelector('.uploadFileError');
					if (errorContainer) {
						errorContainer.textContent = 'Image size exceeds the allowed limit of 8MB.';
